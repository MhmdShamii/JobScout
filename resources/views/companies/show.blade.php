    ...
    <div class="space-y-8">
        ...
        <section class="text-center">
            <div class="flex items-center justify-center mb-4">
                <img src="{{'https://picsum.photos/seed/' . $company->id . '/40/40' }}"
                     alt="{{ $company->name }}"
        ...
        <section class="bg-white/5 rounded-xl p-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                ...
                <div>
                    <div class="text-2xl font-bold text-green-400">{{ $company->jobs->where('featured', true)->count() }}</div>
                    <div class="text-white/70">Featured Jobs</div>
                </div>
                <div>
                    <div class="text-2xl font-bold text-purple-400">{{ $company->jobs->unique('employment_type')->count() }}</div>
                ...
                <div class="space-y-4">
                    @foreach($company->jobs as $job)
                        <x-job-card-wide
                            ...
                            company_url="/companies/{{ $company->id }}"
                            job_url="/jobs/{{ $job->id }}"
                            salary="{{ $job->salary }}"
