<x-layout>
    <x-forms.form method="POST" action="/job/create">
        ...
        <div class="flex items-end gap-4 ">
            <div class="flex-1">
                <x-forms.input name="salary" label="Salary" type="text" />
            </div>
        ...
        <label class="block text-sm font-medium mb-1 text-neutral-300 mt-3">Employment type</label>
        ...
        {{-- Include Alpine init once on the page (if not already) --}}
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('tagPicker', ({
                    ...
                    query: '',
                    options, // [{id, title}, ...]
                    selectedIds: new Set(), // store selected IDs
                    ...
                    // Utility to show selected tag objects (for chips)
                    get selected() {
        ...
        {{-- Tag picker --}}
        <div x-data="tagPicker({ options: @js($tags) })" class="mt-4">
            <label class="block text-sm font-medium mb-1 text-neutral-300">Tags</label>
